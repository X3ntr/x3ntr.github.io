...
The executable or image that is mapped, starts at a certain memory address also known as the **image base address**. In case of *nslookup* this address is `0x7df5eb820000`. The **image base address** is stored in a structure called the [Process Environment Block (PEB)](https://en.wikipedia.org/wiki/Process_Environment_Block), along various other process parameters, data structures, global context, and so on.
